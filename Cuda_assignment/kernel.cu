...
//#endif
#include "C:\Users\AdamM\Downloads\glew-1.11.0\include\GL\glew.h"
#include <GL/gl.h>
#include "C:\Users\AdamM\Downloads\freeglut\include\GL/freeglut.h"
#include <iostream>
using namespace std;
...
__global__ void updateKernel(int* devRead, int* devWrite)
{
	...
	*/
	...
	if (devRead[x * g_windowHeight + y] == HEALTHY || devRead[x * g_windowHeight + y] == CANCER) {
		...
		if (_numSurrounded >= 6) {
			if (devRead[x * g_windowHeight + y] == CANCER)
				HealSurroundingMedicine(devRead[x * g_windowHeight ], devRead[y]);
			else
...
void HealSurroundingMedicine(int x, int y)
{
	...
	*/
	...
	if (x > 0 && y > 0) {
		if (g_quad_write[x - 1][y - 1] == MEDICINE)
			HealSurroundingMedicine(x - 1, y - 1);
	}
	if (y > 0) {
		if (g_quad_write[x][y - 1] == MEDICINE)
	...
	if (x > 0) {
		if (g_quad_write[x - 1][y] == MEDICINE)
	...
	if (button == GLUT_LEFT_BUTTON && state == GLUT_DOWN) {
		...
		// the medicine is not absorbed and propagates radially outwards by one cell
			...
			for (int i = 0; i < maxTopLeft; i++) {
				g_quad_write[x - i][y - i] = MEDICINE;
			}
			for (int i = 0; i < maxTop; i++) {
				g_quad_write[x][y - i] = MEDICINE;
			}
